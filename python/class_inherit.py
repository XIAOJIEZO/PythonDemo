...
class ElectricCar(Car):  # 创建子类时，父类必须包含在当前文件中，且位于子类前面。括号内包含父类名称。
    ...
    def __init__(self, make, model,
                 year):  # 子类的方法__init__()，创建子类的实例时，Python首先需要完成的任务是给父类的所有属性赋值。为此，子类的方法__init__()需要父类施以援手。
        """初始化父类属性"""
        super().__init__(make, model,
                         year)  # super()是一个特殊函数，帮助Python将父类和子类关联起来，这行代码让Python调用父类方法__init__(),让子类实例包含父类所有属性。父类也称为超类。
